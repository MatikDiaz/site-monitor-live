...
def show_notification(message):
    ...
    if not notification_in_progress:
        notification_in_progress = True
        notification_window = tk.Toplevel(root)
        notification_window.title("Notification")

        # Get the required width and height of the window based on the message length
        message_label = tk.Label(notification_window, text=message, wraplength=250)
        message_label.pack(pady=10)
        message_label.update()
        width = message_label.winfo_reqwidth()
        height = message_label.winfo_reqheight()

        # Set the window size based on the message length
        notification_window.geometry(f"{width + 40}x{height + 40}")
        notification_window.resizable(False, False)

        tk.Button(notification_window, text="Close", command=close_notification).pack(pady=5)
